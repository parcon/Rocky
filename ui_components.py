...
def render_training_plan_tab(get_ai_analysis_func):
    ...
    if st.button("Generate Training Analysis"):
        with st.spinner("Analyzing your performance..."):
            analysis = get_ai_analysis_func(st.session_state.pmc_data)
            st.markdown(analysis)

    st.subheader("Projected Training Stress Balance (Form)")
    if not st.session_state.plan_df.empty and not st.session_state.pmc_data.empty:
        ...
        fig_proj = go.Figure()
        fig_proj.add_trace(go.Scatter(x=projected_data['metric_date'], y=projected_data['tsb'], mode='lines', name='Projected TSB', line=dict(color='purple')))
        fig_proj.add_hline(y=0, line_dash="dash", line_color="grey")
        fig_proj.update_layout(title="Projected Form Based on Training Plan", xaxis_title="Date", yaxis_title="TSB (Form)")
        st.plotly_chart(fig_proj, use_container_width=True)
    else:
        st.info("Upload a training plan and process historical data to see your projected TSB.")
